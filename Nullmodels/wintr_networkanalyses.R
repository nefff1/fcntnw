...
f.null.models.wintr <- function (plots, 
                                 int_global, mod) {
    ...
    while (cont) { # if a drawing round does not yield a compatible null matrix
      ...
      int_global_sample <- int_global %>%
        ...
        mutate(ID = paste(Host_Name_std, Herb_Name_std)) %>%
          group_by(Herb_Name_std) %>% 
          mutate(spec_abund = sum(interaction),
                 n_int = n()) %>% 
          ungroup() %>%
      ...
      if (corr_i == "pres.abs"){
        ...
        int_global_raw <- int_global_wintr_raw %>%
          filter(grepl(substr(pl_i, 1, 3), PlotID)) %>% # only consider interactions of the region
          mutate(ID = paste(Host_Name_std, Herb_Name_std)) %>%
          group_by(Herb_Name_std) %>% 
          mutate(n_int = n()) %>% 
  ...
  data.nullmodel <- lapply(plots, f.null.model)
...
stan_formi_mod <- stan_model(file = "scr_stan/Stan_ForMI_model.stan")
stan_formisplit_mod <- stan_model(file = "scr_stan/Stan_ForMIsplit_model.stan")

hier_mod_wintr_null <- function(data, datatype){
  ...
  for (res in responses[responses %in% names(data)]){
    ...
    data$response <- (data[, res] - mean(nw.metrics.wintr[[datatype]][, res]))/sd(nw.metrics.wintr[[datatype]][, res])
    ...
    if (out_full) {
      ...
      out[[res]] <- list(formi_comb = extract(fit)[c("a", "b")])
    }
    ...
    if (out_full) {
      ...
      out[[res]] <- c(out[[res]], list(formi_split = extract(fit)[c("a", "b")]))
    }
    
    
    # cleaning (from https://github.com/stan-dev/rstan/issues/448) -------------.
    dso_filename <-  stan_formi_mod@dso@dso_filename
    ...
    dso_filename <-  stan_formisplit_mod@dso@dso_filename
    loaded_dlls  <-  getLoadedDLLs()
    if (dso_filename %in% names(loaded_dlls)) {
      message("Unloading DLL for model dso ", dso_filename)
      model.dll  <-  loaded_dlls[[dso_filename]][['path']]
...
assign("int_global_target", eval(parse(text  = paste0("int_global_wintr_", gsub("\\.", "_", corr_i)))))

nw.metrics_target <- f.null.models.wintr(plots = plots, 
...
out <- hier_mod_wintr_null(nw.metrics_target, corr_i)

warning(paste(sum(is.na(nw.metrics_target$nested)), "NAs in null models"))

out %>%
  map(cbind.data.frame)  %>%
  enframe("response") %>% 
  unnest() %>% 
  write.table(paste0("out_data/wintr_", gsub("\\.", "_", corr_i), "/data_",
